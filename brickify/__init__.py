...
if sys.version_info < (3,):
    import brickify.images2gif_py2 as images2gif
else:
    import brickify.images2gif_py3 as images2gif
...
def get_new_filename(file_path, ext_override=None):
    ...
    new_filename = os.path.join(folder, "{0}_brick{1}".format(base, extention))
    return new_filename
...
def get_brick_palette(palette_mode):
    """Gets the palette for the specified brick palette mode"""
    bricks = palettes.bricks()
    palette = bricks[palette_mode]
    return palettes.extend_palette(palette)


def apply_thumbnail_effects(image, palette, dither):
    """Apply effects on the reduced image before brickifying"""
    palette_image = Image.new("P", (1, 1))
    palette_image.putpalette(palette)
    return image.im.convert(
        "P", Image.FLOYDSTEINBERG if dither else Image.NONE, palette_image.im
    )
...
def main(image_path, output_path=None, size=None, palette_mode=None, dither=False):
    """Brickify image or gif with brick_path mask"""
    ...
    if palette_mode:
        print("Brick palette {0} selected...".format(palette_mode.title()))
    elif dither:
        palette_mode = "all"
    ...
    if image_path.lower().endswith(".gif") and base_image.is_animated:
        ...
        print("Animated gif detected, will now brickify to {0}".format(output_path))
        brickify_gif(base_image, brick_image, output_path, size, palette_mode, dither)
    else:
        if output_path is None:
            output_path = get_new_filename(image_path, ".png")
        print("Static image detected, will now brickify to {0}".format(output_path))
        brickify_image(base_image, brick_image, output_path, size, palette_mode, dither)
