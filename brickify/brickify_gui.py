...
class BrickifyGui(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.iconbitmap(
            os.path.dirname(os.path.realpath(__file__)) + "/assets/brick.ico"
        )
...
class BrickifyGuiMainFrame(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.pathField = tk.Entry(
            self, width=40, textvariable=self.chosenFilePath, state=tk.DISABLED
        )
        self.pathField.grid(row=0, column=0, padx=10)

        self.selectFile = tk.Button(
            self, text="Choose file...", command=self.choose_a_file
        )
        ...
        self.brickNumberScale = tk.Scale(
            self.groupFrame,
            from_=1,
            to=200,
            orient=tk.HORIZONTAL,
            label="Number of bricks (longer edge)",
            length=250,
        )
        self.brickNumberScale.set(30)
        self.brickNumberScale.grid(
            row=1,
            column=0,
            columnspan=2,
        )

        self.convertFile = tk.Button(
            text="Brickify this image!", command=self.convert_file
        )
        ...
        try:
            if self.chosenFile is not None:
                ...
                if palette in BRICK_PALETTE and palette != "none":
                    brickify.main(
                        self.chosenFile.name,
                        size=self.brickNumberScale.get(),
                        palette_mode=palette,
                    )
                else:
                    brickify.main(
                        self.chosenFile.name, size=self.brickNumberScale.get()
                    )

                tkmsg.showinfo("Success!", "Your image has been brickified!")
            else:
                tkmsg.showerror(
                    "File not found", "Please select a file before brickifying"
                )
